...
-- ----
create function disorder.digsyl(n bigint, l integer)
returns varchar as $$
declare
	result		varchar := '';
	i			integer;
...
begin
	...
	a_customer = disorder.select_nurand_a(n_customer);
	n_item = 400 + 100 * scaling;
	a_item = disorder.select_nurand_a(n_item);
	n_special = 200;
	...
	for order_id in 1 .. n_order loop
		...
		for line_no in 1 .. num_lines loop
			item_id = disorder.nurand(a_item, 1, n_item);
	...
	insert into disorder.do_order
			(o_id, o_c_id, o_order_lines, o_total_items, o_total_value,
			 o_shipped)
		select ol_o_id, disorder.nurand(a_customer, 1, n_customer),
